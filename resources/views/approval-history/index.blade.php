                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body px-0 pt-0 pb-2">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-3 mb-3">
                                                    ...
                                                    <select class="form-control" id="sort_by" name="sort_by">
                                                        @foreach ($filterOptions['sortOptions'] as $value => $label)
                                                            <option value="{{ $value }}"
                                                                {{ request('sort_by', 'approved_at') == $value ? 'selected' : '' }}>
                                                                {{ $label }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div class="col-md-3 mb-3">
                                                    ...
                                                    <select class="form-control" id="per_page" name="per_page">
                                                        <option value="5"
                                                            {{ request('per_page', '15') == '5' ? 'selected' : '' }}>5
                                                        </option>
                                                        <option value="10"
                                                        ...
                                                        <div>
                                                            <span class="text-muted small">
                                                                @if ($approvalHistory->total() > 0)
                                                                    Menampilkan {{ $approvalHistory->firstItem() }} -
                                                                    {{ $approvalHistory->lastItem() }}
                        ...
                        @if ($approvalHistory->count() > 0)
                                    ...
                                    <tbody>
                                        @foreach ($approvalHistory as $approval)
                                            <tr>
                                                    ...
                                                    <div class="d-flex px-2 py-1">
                                                        <div class="d-flex flex-column justify-content-center">
                                                            <h6 class="mb-0 text-sm">
                                                                {{ Str::limit($approval->document->title, 30) }}</h6>
                                                            <p class="text-xs text-secondary mb-0">
                                                                {{ $approval->document->documentType->name }}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <p class="text-xs font-weight-bold mb-0">
                                                        {{ $approval->document->user->name }}</p>
                                                    <p class="text-xs text-secondary mb-0">
                                                        {{ $approval->document->user->email }}</p>
                                                </td>
                                                <td class="align-middle text-center text-sm">
                                                    <span
                                                        class="badge badge-sm bg-gradient-secondary">{{ $approval->level->name }}</span>
                                                </td>
                                                ...
                                                <td class="align-middle text-center">
                                                    <span
                                                        class="text-secondary text-xs font-weight-bold">{{ $approval->approved_at ? $approval->approved_at->format('d/m/Y H:i') : '-' }}</span>
                                                </td>
                                                <td class="align-middle text-center">
                                                    <a href="{{ route('approval-history.show', $approval->id) }}"
                            ...
                            <!-- Modal Inspect PDF -->
                            <div class="modal fade" id="inspectModal" tabindex="-1" aria-labelledby="inspectModalLabel"
                                aria-hidden="true">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const autoSubmitElements = ['status', 'document_type_id', 'division_id', 'level_id', 'sort_by',
            'sort_order', 'per_page'
        ];
